...
<html lang="en">
    ...
    <body>
        ...
        <script>
            ...
            let nickForfree = localStorage.getItem("logedInfo");
            let nickFree_val = JSON.parse(nickForfree); // logedInfo의 별명 사용을 위함

            const BOARDLIST_LS = "freeboards";
            const boardListsObj = [];
            let nums = 1;
            let author = nickFree_val[2];
            let date = new Date();
            let views = 0;

            const HIDDEN_CLASSNAME = "hidden";

            function boardWriteForm() { // 게시글 작성 버튼 동작
                boardList.classList.add(HIDDEN_CLASSNAME); // 목록 숨기기
                boardWrite.classList.remove(HIDDEN_CLASSNAME); // 게시글 작성란 보이기
            }

            function returnBoard() { // 돌아가기 버튼 동작
                boardList.classList.remove(HIDDEN_CLASSNAME); // 목록 보이기
                boardDetail.classList.add(HIDDEN_CLASSNAME); // 선택된 게시물 화면 숨기기
            }
            ...
            function onEditorFormSubmit(e) { // 게시글 등록
                e.preventDefault();
            ...
            function showBoardLists() { // 게시글 목록
                const lists = JSON.parse(localStorage.getItem(BOARDLIST_LS));
            ...
            function showBoardListsNewPage(pageIndex) { // 게시글 5개 이상일때 페이지 생성 (5개 단위 생성)
